...
$stmt = $db->prepare("
select * from VIEW_PILOTS group by id_profile
	LIMIT :offset, :limit;
");
...
<div class="col mt-3">
	<div class="card bg-clair mb-3" style="max-width: 100%;">
				<div class="row g-0 ">
					...
					<div class="col-md-10">
						<div class="card-body sombre row">
							...
							<div class="col text-end list-item" id="<?=  $row['id_profile'] ?>">
								<button class="rounded-4 but clair px-2 py-2" type="button" data-bs-toggle="collapse" data-bs-target="#pil<?=  $row['id_profile'] ?>" aria-expanded="false" aria-controls="pil<?=  $row['id_profile'] ?>" style="width:max-content;">
									Voir les promos
								</button>
							</div>
						</div>
					...
					<div class="collapse" id="pil<?=  $row['id_profile'] ?>">
