CREATE DEFINER=`root`@`localhost` PROCEDURE `SEARCH_PROFILE`(p_id_profile INT)
BEGIN
SELECT profile.id_profile, profile.fname, profile.lname, profile.role, prom.id_prom, prom.name as promo, campus.id_campus, campus.name as campus, city.name as city
FROM profile
